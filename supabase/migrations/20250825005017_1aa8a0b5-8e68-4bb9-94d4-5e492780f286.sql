-- Corrigir função toggle_user_admin_status para resolver ambiguidade de user_id
...
BEGIN
    ...
    -- Get current admin status and email (using explicit table aliases to avoid ambiguity)
    SELECT COALESCE(p.is_admin, false), COALESCE(u.email, p.first_name || '@***')
    INTO current_status, target_user_email
    FROM public.profiles p
    LEFT JOIN auth.users u ON u.id = p.id
    WHERE p.id = target_user_id; -- Usar target_user_id em vez de user_id
    ...
    SET 
        is_admin = make_admin,
        updated_at = now()
    WHERE id = target_user_id; -- Usar target_user_id em vez de user_id
