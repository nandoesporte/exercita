...
-- Create regras_resposta_automatica table
CREATE TABLE public.regras_resposta_automatica (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Create workout_categories table (needed by the app)
...
-- Create exercises table (needed by the app)
...
-- Create workouts table (needed by the app)
...
-- Create workout_exercises table (needed by the app)
...
-- Create appointments table (needed by the app)
...
-- Create pix_keys table (needed by the app)
...
-- Create payment_history table (needed by the app)
...
ALTER TABLE public.regras_resposta_automatica ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Admin users can view all admin users" ON public.admin_users FOR SELECT USING (auth.uid() IN (SELECT user_id FROM public.admin_users WHERE active = true));

-- Create RLS policies for categories (public read)
CREATE POLICY "Categories are viewable by everyone" ON public.categories FOR SELECT USING (true);

-- Create RLS policies for site_config (public read)
CREATE POLICY "Site config is viewable by everyone" ON public.site_config FOR SELECT USING (true);

-- Create RLS policies for workout_categories (public read)
...
-- Create RLS policies for workouts (public read)
CREATE POLICY "Workouts are viewable by everyone" ON public.workouts FOR SELECT USING (true);

-- Create RLS policies for workout_exercises (public read)
CREATE POLICY "Workout exercises are viewable by everyone" ON public.workout_exercises FOR SELECT USING (true);

-- Create RLS policies for trainings (public read)
CREATE POLICY "Trainings are viewable by everyone" ON public.trainings FOR SELECT USING (true);

-- Create RLS policies for appointments
CREATE POLICY "Users can view their own appointments" ON public.appointments FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can create their own appointments" ON public.appointments FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can update their own appointments" ON public.appointments FOR UPDATE USING (auth.uid()::text = user_id::text);

-- Create RLS policies for pix_keys
CREATE POLICY "Users can view their own pix keys" ON public.pix_keys FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can create their own pix keys" ON public.pix_keys FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can update their own pix keys" ON public.pix_keys FOR UPDATE USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can delete their own pix keys" ON public.pix_keys FOR DELETE USING (auth.uid()::text = user_id::text);

-- Create RLS policies for payment_history
CREATE POLICY "Users can view their own payment history" ON public.payment_history FOR SELECT USING (auth.uid()::text = user_id::text);
...
CREATE TRIGGER update_regras_resposta_automatica_updated_at BEFORE UPDATE ON public.regras_resposta_automatica FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER update_workout_categories_updated_at BEFORE UPDATE ON public.workout_categories FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER update_pix_keys_updated_at BEFORE UPDATE ON public.pix_keys FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
